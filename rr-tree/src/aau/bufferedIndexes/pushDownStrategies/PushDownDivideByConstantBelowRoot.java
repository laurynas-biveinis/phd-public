...
/**
 * A push down strategy that uses divides group size by the global average node fanout at levels below root
 */
public class PushDownDivideByConstantBelowRoot<E extends Convertable> extends AbstractPushDownThreshold<E> {

    private final PushDownGroupsStrategy<E> pushDownAtRoot;
    private final IRRDiskTree<E> tree;
    private final double coefficient;

    public PushDownDivideByConstantBelowRoot(final IRRDiskTree<E> tree, final PushDownGroupsStrategy<E> pushDownAtRoot,
    ...
    public PushDownAndBufferGroups<E> choosePushDownGroups(final IndexEntryOpGroupMap<E> groupsToFilter,
        ...
        int divider = 1;
        for (int i = 0; i < tree.height() - childNodeLevel - 1; i++) {
            divider *= (tree.getMinNodeCapacity() + tree.getMaxNodeCapacity()) / 2.0D * coefficient;
        }
        return applyThreshold(groupsToFilter.numOfDistinctOps() / divider, groupsToFilter);
    }

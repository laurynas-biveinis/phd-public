...
/**
 * A push down strategy that uses constant threshold below root
 */
public class PushDownThresholdBelowRoot<E extends Convertable> extends AbstractPushDownThreshold<E> {

    private final PushDownGroupsStrategy<E> pushDownAtRoot;
    private final PushDownThreshold<E> pushDownBelow;
    private final IRRDiskTree<E> tree;

    public PushDownThresholdBelowRoot(final IRRDiskTree<E> tree, final PushDownGroupsStrategy<E> rootStrategy,
                                      final int threshold, final boolean groupSizeByInsertions) {
        super(threshold, groupSizeByInsertions);
        pushDownAtRoot = rootStrategy;
        pushDownBelow = new PushDownThreshold<>(threshold, groupSizeByInsertions);
        this.tree = tree;
    }
    ...
    public PushDownAndBufferGroups<E> choosePushDownGroups(final IndexEntryOpGroupMap<E> groupsToFilter,
        ...
        if (tree.height() == childNodeLevel + 1)
            return pushDownAtRoot.choosePushDownGroups(groupsToFilter, childNodeLevel, childNodeSize,
                    calledFromRestart);
        return pushDownBelow.choosePushDownGroups(groupsToFilter, childNodeLevel, childNodeSize, calledFromRestart);
    }
    ...
    public int getThresholdSatisfactions() {
        return pushDownBelow.getThresholdSatisfactions();
    }

    public int getThresholdUnsatisfactions() {
        return pushDownBelow.getThresholdUnsatisfactions();
    }
    ...
    public boolean willEmptyBigPartOfBuffer() {
        return pushDownAtRoot.willEmptyBigPartOfBuffer();
    }
}

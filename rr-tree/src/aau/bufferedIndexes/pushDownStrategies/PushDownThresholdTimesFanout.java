...
/**
 * A push down strategy that uses a constant threshold one above leaf level and multiplies it by global fanout on each
 * higher level.
 */
public class PushDownThresholdTimesFanout<E extends Convertable> extends AbstractPushDownThreshold<E> {

    private final IRRDiskTree<E> tree;

    public PushDownThresholdTimesFanout(final IRRDiskTree<E> tree, final int oneAboveLeafThreshold,
                                        final boolean groupSizeByInsertions) {
        super(oneAboveLeafThreshold, groupSizeByInsertions);
        this.tree = tree;
    }
    ...
    public PushDownAndBufferGroups<E> choosePushDownGroups(final IndexEntryOpGroupMap<E> groupsToFilter,
        ...
        int t = threshold;
        for (int i = 0; i < childNodeLevel - 1; i++) {
            t *= (tree.getMinNodeCapacity() + tree.getMaxNodeCapacity()) / 2;
        }
        return applyThreshold(t, groupsToFilter);
    }

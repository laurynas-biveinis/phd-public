...
public class XTree extends RTree {
	...
	public class Node extends RTree.Node {
		...
		/* (non-Javadoc)
		 * @see xxl.core.indexStructures.ORTree.Node#chooseSubtreeByDescriptor(xxl.core.indexStructures.Descriptor, java.util.Iterator)
		 */
		...
		public class SplitInfo extends RTree.Node.SplitInfo {
			 ...
			 */
			public ORTree.Node.SplitInfo initialize( RTree.Distribution distribution) {
		...
		protected Tree.Node.SplitInfo split (Stack path) {
			...
			RTree.Distribution distribution = ((RTree.Node.SplitInfo)splitInfoTopo).distribution();
			...
			if (level() != 0) {
				...
				if (relOverlap > maxOverlap) {
					...
					Distribution distr = splitInfo.distribution();
					...
					public List<Distribution> invoke(final Integer dim) {
						...
						for (int i=0; i<2; i++) {
							...
							for (int j = minEntries; j<=maxEntries; j++)
                                throw new UnsupportedOperationException("Please port changes from RTree.Distribution!");
//								distributionList.add(new Distribution(entryArray, j, rectangles[0][j-minEntries], rectangles[1][maxEntries-j], dim));
			...
			List<Distribution> distributionList = (List<Distribution>)(Cursors.minima(
				distributionLists,
				new Function () {
					public Object invoke (Object l) {
                        final List<Distribution> list = (List<Distribution>)l;

...
public abstract class Tree {
	...
	*/
	...
	/**
     * Initializes the tree. Both Function objects <tt>getContainer</tt> and <tt>determineContainer</tt> are
	...
	public Tree initialize (IndexEntry rootEntry, Descriptor rootDescriptor, Function getDescriptor, Container container, final int minCapacity, final int maxCapacity) {
		return initialize(
			...
			new Function () {
				public Object invoke (Object node) {
					return minCapacity / (double) ((Node) node).number();
				}
			},
			new Function () {
				public Object invoke (Object node) {
					return 1.0 - minCapacity / (double) ((Node) node).number();
				}
			}
		);
	}
	...
	public Tree initialize (IndexEntry rootEntry, Descriptor rootDescriptor, Function getDescriptor, Container container, final int innerNodeMinCapacity, final int innerNodeMaxCapacity, final int leafMinCapacity, final int leafMaxCapacity) {
		return initialize(
			...
			new Function () {
				public Object invoke (Object node) {
					Tree.Node treeNode = (Node) node;
					return (treeNode.level() == 0 ? leafMinCapacity : innerNodeMinCapacity) / (double) treeNode.number();
				}
			},
			new Function () {
				public Object invoke (Object node) {
					Tree.Node treeNode = (Node) node;
					return 1.0 - (treeNode.level() == 0 ? leafMinCapacity : innerNodeMinCapacity) / (double) treeNode.number();
				}
			}
		);
	}
	...
	public Tree initialize (Function getDescriptor, int blockSize, Container container, int dataSize, int descriptorSize, double minMaxFactor) {
		...
		int leafMinCap = (int) (leafMaxCap * minMaxFactor);
		int innerNodeMinCap = (int) (innerNodeMaxCap * minMaxFactor);			
		return initialize(getDescriptor, container, innerNodeMinCap, innerNodeMaxCap, leafMinCap, leafMaxCap);
	}
	...
	public class IndexEntry {
		...
		*/
		...
		public IndexEntry chooseSubtree (Descriptor descriptor, Stack path) {
			return down(path, this).chooseSubtreeByDescriptor(descriptor, path);
		}
	...
	*/
	public abstract class Node {
		...
		*/
		 ...
		 * the underlaying buffer. The created index entries will be posted into the parent node.
		 * @param path the path from the root to the overflowing node
		 ...
		 * It splits the overflowing node in new nodes and creates new index-entries for 
		 * the new nodes. These index-entries will be returned as a <tt>Collection</tt>.
         * The created index entries will be posted into the parent node.
		 * @param path the path from the root to the overflowing node
		...
		*/
		public abstract Iterator descriptors();
		...
		*/
		 ...
		 */
		protected abstract IndexEntry chooseSubtreeByDescriptor(Descriptor descriptor, Stack path);
		...
		*/
		protected double splitMinRatio () {
			return (Double) getSplitMinRatio.invoke(this);
		}
		...
		*/
		protected double splitMaxRatio () {
			return (Double) getSplitMaxRatio.invoke(this);
		}
	...
	public class Query implements Iterator {
			...
			*/
			...
			public String toString(){
				...
				if(entry instanceof IndexEntry) {
                    sb.append("entry : id ");
                    sb.append(((IndexEntry)entry).id());
                    sb.append("  parentlevel ");
                    sb.append(((IndexEntry)entry).level());
                }
				sb.append("\n  descriptor ");
		...
		*/
		...
		public Query (Queue queue, int targetLevel) {
			...
			if (rootEntry()!=null)
				queue.enqueue(createCandidate(rootEntry(), rootDescriptor(), height()));
		}
		 ...
		 */
		public Candidate createCandidate(Object entry, Descriptor descriptor, int level) {
		 ...
		 */
		protected Iterator expand(final Node node) {

			return new Mapper(
				new Function () {
					public Object invoke (Object entry, Object descriptor) {
						return createCandidate(entry, (Descriptor)descriptor, node.level);
					}
				}
			,node.entries(), node.descriptors());
		}
		...
		public boolean hasNext () {
			for (; !queue.isEmpty() && ((Candidate)queue.peek()).parentLevel!=targetLevel;) {
				...
				if (node.level>=targetLevel)
					Queues.enqueueAll(queue, expand(node));
			}

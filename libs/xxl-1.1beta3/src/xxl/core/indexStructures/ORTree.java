...
public abstract class ORTree extends Tree {
	...
	/**
	 * Initializes the tree with the root descriptor.
	 * 
	 * @param rootEntry the new {@link Tree#rootEntry}
	 * @param getDescriptor the new {@link Tree#getDescriptor}
	 * @param container used as constant for {@link Tree#determineContainer}
	...
	public Descriptor computeDescriptor (Collection collection) {
		...
		if (entries.hasNext()) {
            descriptor = new DoublePointRectangle((Rectangle)descriptor(entries.next()));
	...
	public abstract class Node extends Tree.Node {
		...
		*/
		...
		public Iterator descriptors() {
            ...
            final Collection<Object> coll = new ArrayList<Object>(entries.size());
            if (level == 0) {
                for (final Object o : entries) {
                    //noinspection unchecked
                    coll.add(getDescriptor.invoke(o));
                }
            }
            else {
                //noinspection unchecked
                for (final IndexEntry e : (Collection<IndexEntry>)entries) {
                    coll.add(e.descriptor);
                }
            }
		...
		*/
		public Iterator query (final Descriptor queryDescriptor) {
            ...
            if (level == 0)
                filterPredicate = new Predicate() {
                    public boolean invoke (final Object object) {
                        // TODO: generify properly
                        //noinspection unchecked
                        return ((Descriptor)getDescriptor.invoke(object)).overlaps(queryDescriptor);
                    }
                };
            else
                filterPredicate = new Predicate() {
                    public boolean invoke (final Object entry) {
                        return ((IndexEntry)entry).descriptor.overlaps(queryDescriptor);
                    }
                };
            //noinspection unchecked
            return new Filter(entries(), filterPredicate);
		}
		 ...
		 */
		protected Tree.IndexEntry chooseSubtreeByDescriptor(Descriptor descriptor, Stack path) {
			IndexEntry indexEntry = chooseSubtreeByDescriptor(descriptor);
		 ...
		 * @return the index entry refering to the root of the chosen subtree
		 */
		protected abstract IndexEntry chooseSubtreeByDescriptor(Descriptor descriptor);
	...
	public class NodeConverter extends Converter {
		...
		public void write (DataOutput dataOutput, Object object) throws IOException {
			...
			ShortConverter.DEFAULT_INSTANCE.write(dataOutput, (short) node.level);
			IntegerConverter.DEFAULT_INSTANCE.write(dataOutput, node.number());
			...
			public boolean hasNextObject() {
                if (!initialized) {
                    if (height()>0 && queryDescriptor.overlaps(rootDescriptor()))
                        iterators[height()] = new SingleObjectCursor<Tree.IndexEntry>(rootEntry());
                    initialized = true;
                }
				...
				if (targetLevel<height()) {
					...
					for (;;) {
						...
						if (node.number()==0) {
							...
							indexEntry = (IndexEntry)node(path).chooseSubtreeByDescriptor(indexEntry.descriptor(), path);

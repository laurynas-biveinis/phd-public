...
public class BufferedRandomAccessFile extends RandomAccessFile {
	...
	protected void getBlock (boolean readBlock) throws IOException {
		if (block!=null && filePointer/blockSize!=blockId) {
			buffer.unfix(this, Long.valueOf(blockId));
			block = null;
		}
		if (block==null) {
			...
			try {
				block = readBlock?
					(byte[])buffer.get(this, Long.valueOf(blockId),
						new Function () {
	...
	public void setLength (long newLength) throws IOException {
		...
		if (newLength<length) {
			...
			for (long id = (length-1)/blockSize; id>(newLength-1)/blockSize; id--)
				buffer.remove(this, Long.valueOf(id));
		}
		length = newLength;
		if (filePointer>length)
			filePointer = length;
	...
	public void write (byte [] array, int off, int len) throws IOException {
		while (len>0) {
			...
			try {
				buffer.update(this, Long.valueOf(blockId), block,
					new Function () {

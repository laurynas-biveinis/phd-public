	...
	public AppendedResultSetMetaData(ResultSetMetaData... metaData) {
		...
		try {
			...
			for (int i = 0; i < this.metaData.length; i++) {
				...
				for (int j = 1; j <= columnCounts[i]; j++) {
					columnName = metaData[i].getColumnName(j);
					if (hashSet.contains(columnName))
						throw new MetaDataException("specified meta data information cannot be appended because not all columns have different names");

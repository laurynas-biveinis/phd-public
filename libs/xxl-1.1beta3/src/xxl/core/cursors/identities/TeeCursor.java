 ...
 * Then, only a call to {@link #closeAll()} closes the input and the StorageArea.</p>
 * 
...
public class TeeCursor<E> extends AbstractCursor<E> {
	...
	public static class FileStorageArea<E> implements StorageArea<E> {
		...
		public void close() {
			try {
			    ...
			    boolean result = file.delete(); // deleting file
                assert result;
            }
		}
	}
	...
	public static void main(String[] args) {
		...
		while (numberOfFullyConsumedCursors < numberOfCursors) {
			cursorNumber = random.nextInt(numberOfCursors);

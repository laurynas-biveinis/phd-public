...
public class RTreeTest {
	 ...
	 */
	static Function<Object, KPE> LEAFENTRY_FACTORY = new Function<Object, KPE>() {
		public KPE invoke() {
	 ...
	 */
	static Function GET_DESCRIPTOR = new Function () {
	...
	public static void main (String args[]) throws Exception {
		...
		int dataSize = 32+4; // DoublePointRectangle + int
		...
		CounterContainer lowerCounterContainer = new CounterContainer(
			new ConverterContainer(
				fileContainer,
				rtree.nodeConverter(
					new ConvertableConverter(LEAFENTRY_FACTORY),2
				)
			)
		);
		...
		universe = Rectangles.readSingletonRectangle(new File(Common.getDataPath()+filenameTree+".universe"), new DoublePointRectangle(2));
		...
		if (tree.equalsIgnoreCase("x")) {
			...
			((XTree) rtree).initialize(GET_DESCRIPTOR, container, xTreeMinCap, xTreeMaxCap, 2);
		}
		else
		...
		if (!bulk) {
			...
			if (showData) {
				ShowKPERectanglesCursor rc = new ShowKPERectanglesCursor(cursor,universe,showDataDelay);

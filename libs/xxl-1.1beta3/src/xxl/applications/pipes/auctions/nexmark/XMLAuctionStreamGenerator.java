...
*/
// The Code has been changed to provide three separated streams.
//
...
class XMLAuctionStreamGenerator {
    ...
    // CHANGES start
	private MyBuffer personBuffer;
	private MyBuffer auctionBuffer;
	private MyBuffer bidBuffer;
	private BufferedWriter personWriter;
	private BufferedWriter auctionWriter;
	private BufferedWriter bidWriter;
    // CHANGES end    
    ...
    //CHANGED
	public void generateStream(BufferedWriter personWriter, BufferedWriter auctionWriter
		,BufferedWriter bidWriter) throws IOException {
			
		personBuffer = new MyBuffer();
		auctionBuffer = new MyBuffer();
		bidBuffer = new MyBuffer();
		this.personWriter = personWriter;
		this.auctionWriter = auctionWriter;
		this.bidWriter = bidWriter;
		...
		// put 10 persons in a document
		for(int i = 0;i<5; i++) {
			initBuf(personBuffer);
			for(int j = 0; j<10; j++) {
				generatePerson(personBuffer, 1);
			}
			writeBuf(personBuffer, personWriter);
		}

		for(int i = 0; i<5; i++) {
			initBuf(auctionBuffer);
			for(int j = 0; j<10; j++) {
				generateOpenAuction(auctionBuffer, 1);
			}
			writeBuf(auctionBuffer, auctionWriter);
		}
		...
		while(count < numGenCalls) {
			...
			// want on average 1 item and 10 bids
			int numItems = rnd.nextInt(3); // should average 1
			generateOpenAuction(auctionBuffer, numItems);
	    
			int numBids = rnd.nextInt(21); // should average 10
			generateBid(bidBuffer, numBids);
	...
	private void initAllBuf() throws IOException {
		personBuffer.clear();
		personBuffer.append(XML_DECL);
		personBuffer.append("<site>");
		personBuffer.append(nl);
		
		auctionBuffer.clear();
		auctionBuffer.append(XML_DECL);
		auctionBuffer.append("<site>");
		auctionBuffer.append(nl);
		
		bidBuffer.clear();
		bidBuffer.append(XML_DECL);
		bidBuffer.append("<site>");
		bidBuffer.append(nl);
	}
    
	// CHANGED	
	private void writeAllBuf() throws IOException {
		personBuffer.append("</site>");
		personBuffer.append(nl);
		personWriter.write(personBuffer.array(), 0, personBuffer.length());
		
		auctionBuffer.append("</site>");
		auctionBuffer.append(nl);
		auctionWriter.write(auctionBuffer.array(), 0, auctionBuffer.length());

		bidBuffer.append("</site>");
		bidBuffer.append(nl);
		bidWriter.write(bidBuffer.array(), 0, bidBuffer.length());

	}	
	
	private void initBuf(MyBuffer myBuf) throws IOException {
		myBuf.clear();
		myBuf.append(XML_DECL);
		myBuf.append("<site>");
		myBuf.append(nl);
	...
	// ORGINAL
	private void writeBuf(MyBuffer myBuf, BufferedWriter writer) throws IOException {
		myBuf.append("</site>");
		myBuf.append(nl);
		writer.write(myBuf.array(), 0, myBuf.length());
	} 	

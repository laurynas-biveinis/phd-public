...
// We now allow negative timestamps		
//		if (start < 0 || end < 0)
//			throw new IllegalArgumentException("Negative intervals are not permitted: ["+start+", "+end+").");
	...
	/**
	 * Checks the overlap with an interval.
	 * @param start start of the given interval
	...
	public TimeInterval intersect(TimeInterval t) throws IllegalArgumentException {
		if(!overlaps(t))
			throw new IllegalArgumentException("Disjoint intervals.");
		return new TimeInterval(Math.max(start, t.start), Math.min(end, t.end));
	}
	
	/**
	 * Returns the intersection of overlapping intervals.
	 * @param start start of the given interval
	 * @param end end of the given interval
	 * @return intersection 
	 * @throws IllegalArgumentException
	 */
	public TimeInterval intersect(long start, long end) throws IllegalArgumentException {
		if(!overlaps(start, end))
			throw new IllegalArgumentException("Disjunct intervals.");
		return new TimeInterval(Math.max(this.start, start), Math.min(this.end, end));
	}

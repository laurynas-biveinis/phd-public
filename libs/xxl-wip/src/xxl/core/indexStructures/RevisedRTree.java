...
//			 chooseSubTree aktulisiert den Descriptor von IndexEntry von Übergelaufenen Knoten	
			...
			if (node.level()> 0){ //inner Nodes
				...
				Function<List<RevisedRTree.Node.Distribution>, Double> getMarginOfDistr = new AbstractFunction
					<List<RevisedRTree.Node.Distribution>, Double>(){
					public Double invoke (List<RevisedRTree.Node.Distribution> list) {
		...
		protected Iterator<Distribution> computeCostsDistr(final Iterator<List<Distribution>> listIterator, final Node node, final IndexEntry indexEntry){
			...
			} , new Sequentializer<Distribution>(new Mapper<List<Distribution>, Iterator<Distribution> >(new AbstractFunction<List<Distribution>, Iterator<Distribution> >(){ 
						public  Iterator<Distribution> invoke(List<Distribution> list){

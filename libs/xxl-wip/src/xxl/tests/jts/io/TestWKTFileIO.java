 ...
 * of the class WKTFileIO.
 */
public class TestWKTFileIO {
	 ...
	 * 					the first parameter specifies the name of the WKT- file to read; 
	...
	public static void main(String[] args){
		if(args.length == 0){
			...
			System.out.println("usage: java xxl.connectivity.jts.io.WKTFileIO wkt-File [bufferSize]");
			return;
		}
		
		File file1 = new File(args[0]);
		int bufferSize = args.length > 1  ? Integer.parseInt(args[1]) : 4096;
		Iterator<Geometry2D> it = WKTFileIO.read(file1, bufferSize);
